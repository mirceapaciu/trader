...
For an assignment to an existing window or card, incompatible classified event identities (family, subject, fiscal period, or stable identifier) are removed before lexical or semantic assignment. Equal deterministic event-instance keys are the same occurrence. Incomplete or unmapped identities remain eligible for semantic fallback; missing fields never imply a mismatch. A window's seeded identity and a published card's identity are immutable.

For an assignment to an existing window or card, two or more distinctive shared story tokens are a deterministic positive signal. Zero or one shared token is lexically inconclusive, not proof of a different event: when the event check is enabled, ThesisBuilder asks whether both articles describe the same underlying occurrence. A `same` result preserves the chosen target; a `different` result resolves to `new_story` with `story_event_mismatch`. If a zero-overlap check is unavailable, malformed, budget-exhausted, or fails, ThesisBuilder safely resolves to `new_story` with `story_event_check_unavailable` and `event_check=unavailable`. This policy is shared by live processing, reprocessing, and regeneration because all use the same repository verification path. Event checks add one bounded LLM call for each zero- or one-overlap existing-target assignment; their usage is charged to the existing ThesisBuilder LLM budget.
