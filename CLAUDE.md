...
**Shared contracts**: components must not query other components' schemas directly. Cross-cutting access goes through `src/product_components/shared/adapters.py`: `PostgresSharedInstrumentRegistry`, `PostgresSharedInstrumentAdmin`, `PostgresSharedApiUsageWriter`, `PostgresSharedThesisCardReviewWriter`. Reading another *product* component's data (e.g. the Backtester consuming ThesisBuilder analyses/cards, or MarketData bars) goes through that component's owned read/export contract — `ThesisCardHistoryExporter`, `ThesisAnalysisHistoryExporter`, `MarketDataService.get_historical_bars` — never raw SQL against the foreign schema. This applies to offline report/CLI tools too: a report that opens its own `psycopg` connection and interpolates a foreign schema name is a boundary violation even though it only reads. Every data-reading component ships a `tests/product_components/<component>/test_boundary.py` that statically asserts its source contains no `FROM <foreign_schema>.t_` SQL; add one when creating a new component (the guard is what makes a copied-template violation fail loudly instead of silently).
