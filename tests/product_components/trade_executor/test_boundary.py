...
_IB_IMPORT = re.compile(r"^\s*(?:import\s+ib_insync|from\s+ib_insync\b)", re.MULTILINE)


def test_ib_insync_imported_only_in_the_gateway_module() -> None:
    """ib_insync must be isolated to broker/ib_insync_gateway.py.
    ...
    for path in package.rglob("*.py"):
        if path.name == "ib_insync_gateway.py":
            continue
        text = path.read_text(encoding="utf-8")
        if _IB_IMPORT.search(text):
            offenders.append(str(path.relative_to(package)))
    assert offenders == [], f"ib_insync imported outside the gateway module: {offenders}"
