...
-- Point-in-time company fundamentals (append-only). A new row is inserted only
-- when the fetched values differ from the latest stored row; unchanged refreshes
-- just advance last_checked_at. Regeneration selects the latest row with
-- fetched_at <= analysis time to avoid look-ahead.
CREATE TABLE IF NOT EXISTS market_data.t_instrument_fundamentals (
    ...
    revenue_ttm_usd DOUBLE PRECISION,
...
CREATE INDEX IF NOT EXISTS ix_instrument_fundamentals_instrument_fetched
    ON market_data.t_instrument_fundamentals (ticker, exchange_code, fetched_at DESC);
